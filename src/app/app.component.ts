...
export class AppComponent {
  ...
  electricCredit: Charge = new Charge(0, this.year, this.electricCreditStr, '-35.73');
  gasCredit: Charge = new Charge(0, this.year, this.gasCreditStr, '-27.18');

  private addPerson(): void {
    this.people.push(new Person(this.people.length, this.year, ''));
  }

  private deletePerson(index): void {
  ...
  private addCharge(name: string = ''): void {
    this.charges.push(new Charge(this.charges.length, this.year, name, ''));
  }

  private deleteCharge(index: number): void {
  ...
  private totalDays(charge: Charge): number {
    let total = 0;
  ...
  private personChargeAmounts(person: Person): number[] {
    return this.charges.map(charge => person.payForCharge(charge, this.totalDays(charge)));
  }

  private personTotalAmount(person: Person): Equation {
  ...
  private personWithGuestsTotalAmount(person: Person): Equation {
...
//   Tarence -28.25 * (29 / (365 / 2)) * 4.80/31.39 = -0.69
  private personElectricCredit(person: Person): Equation {
  ...
  private personGasCredit(person: Person): Equation {
    let gasAmount = 0;
    let durationInDays = 0;
